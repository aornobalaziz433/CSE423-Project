...
def update_game_state():
    ...
    for z in zombies:
        ...
        target_pos = car_pos
        if clone_active:
            dist_to_player = dist2d(z['pos'], car_pos)
            dist_to_clone = dist2d(z['pos'], clone_pos)
            if dist_to_clone < dist_to_player:
                target_pos = clone_pos
        dx = target_pos[0] - z['pos'][0]
        dy = target_pos[1] - z['pos'][1]
        dist = math.hypot(dx, dy)
        if dist > 0:
            z['pos'][0] += (dx / dist) * props['speed']
            z['pos'][1] += (dy / dist) * props['speed']
...
def keyboardListener(key, x, y):
    global car_angle, last_w_time, last_s_time, current_camera_mode, clone_active, LIVES, clone_hits
    if key == b'w':
...
def initialize_world():
    ...
    global clone_active, clone_hits
    LIVES = 9
    SCORE = 0
    COIN_COUNT = 0
    game_over = False
